require(["models", "react", "todos"], function(Models, React, Data){
    ...
    componentWillUpdate: function(nextProps, nextState){
    	console.log('we need to diff the current and nextstates');
    	/*
		IDEA:
		-diff current and next states
		-insert the objects that are present in the new state but not the old
		-delete the objects that are present in the old state but not the new
		-update the objects that are changed in between the old and new state
    	*/
    	...
    	$.ajax({
          ...
          data: {"todos"  : JSON.stringify(nextState.todos), 
      			 "delete" : JSON.stringify(deleted_todos),
      			 "update" : JSON.stringify(updated_todos)},
          success: function(data){
    ...
    React.render(
      React.createElement(ToDoList, {url: "/todo", poll: 100000, data: Data.ToDoCollection}),
